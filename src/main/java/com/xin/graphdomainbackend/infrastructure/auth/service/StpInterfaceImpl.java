package com.xin.graphdomainbackend.infrastructure.auth.service;
...
import com.xin.graphdomainbackend.infrastructure.auth.model.SpaceUserAuthContext;
...
import com.xin.graphdomainbackend.spaceuser.dao.entity.SpaceUser;
...
import static com.xin.graphdomainbackend.user.constant.UserConstant.BAN_ROLE;
import static com.xin.graphdomainbackend.user.constant.UserConstant.USER_LOGIN_STATE;
...
public class StpInterfaceImpl implements StpInterface {
    ...
    private SpaceUserAuthContext getAuthContextByRequest() throws IOException {
        ...
        if (ContentType.JSON.getValue().equals(contextType)) { // 如果是post请求
            ...
            // 转成 <String,String>
            Map<String, String> simpleMap = paramMap.entrySet().stream()
                    .collect(Collectors.toMap(
                            Map.Entry::getKey,
                            e -> e.getValue() != null && e.getValue().length > 0 ? e.getValue()[0] : null
                    ));
            authRequest = BeanUtil.toBean(simpleMap, SpaceUserAuthContext.class);
        }
